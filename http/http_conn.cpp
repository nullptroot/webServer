...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text)//这个也在8.6
{
    ...
    *m_url++ = '\0';
    ...
    if (strcasecmp(method, "GET") == 0)//被截断了，所以时可以直接比较的
        m_method = GET;
    else if (strcasecmp(method, "POST") == 0)
    {
        ...
        return BAD_REQUEST;//这一套条件判断是来解析请求方法的
    
    m_url += strspn(m_url, " \t");//这个相反，返回不是' '和'\t'的下标 可能请求行分割符有很多个
    m_version = strpbrk(m_url, " \t");//可以直接用时因为上面有个后缀++
    //m_url开始 m_version下一个' ' '\t'
    if (!m_version)
        return BAD_REQUEST;
    *m_version++ = '\0';
    m_version += strspn(m_version, " \t");//m_version 直接跳过url 定位后面的http版本号了
    if (strcasecmp(m_version, "HTTP/1.1") != 0)
        return BAD_REQUEST;
    if (strncasecmp(m_url, "http://", 7) == 0)//去测试的  完全没有这个东西  可能有的请求时绝对路径，有的时相对路径 相对路径时没有这个东西的
    {
        m_url += 7;
        m_url = strchr(m_url, '/');
    }

    if (strncasecmp(m_url, "https://", 8) == 0)//如果有这个东西，就直接找到最后面的文件内容
    {
        m_url += 8;
        m_url = strchr(m_url, '/');
    }
...
http_conn::HTTP_CODE http_conn::parse_headers(char *text)
{
    if (text[0] == '\0')
    {
        ...
        return GET_REQUEST;//这个标识读完头了 没有内容要读
    }
...
http_conn::HTTP_CODE http_conn::process_read()
{
    ...
    while ((m_check_state == CHECK_STATE_CONTENT && line_status == LINE_OK) || ((line_status = parse_line()) == LINE_OK))
    {
        ...
        case CHECK_STATE_REQUESTLINE:
        {
            ...
            //读完第一行后  设置m_check_state为CHECK_STATE_HEADER
            if (ret == BAD_REQUEST)
                return BAD_REQUEST;
            break;
        }
        case CHECK_STATE_HEADER:
        {
            ...
            //把请求头处理完后，才会设置m_check_state为CHECK_STATE_CONTENT 没读完就一直一行一行的读
            if (ret == BAD_REQUEST)
                return BAD_REQUEST;
            else if (ret == GET_REQUEST)//标识没有内容了，请求告一段落
            {
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    if(cgi == 1 && *(p+1)=='2' || *(p+1) == '3')
    {
        ...
        strncpy(m_real_file+len,m_url_real,FILENAME_LEN-len-1);/*拼接好了文件路径和位置*/
        free(m_url_real);
...
void http_conn::process()
{
    ...
    if (read_ret == NO_REQUEST)//NO_REQUEST 应该就是内容没有读完
    {
