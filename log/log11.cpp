...
Log::Log():m_buf(nullptr,[](char *x){delete [] x;})
{
    m_count = 0;
    m_is_async = false;
}
...
bool Log::init(const std::string &file_name,int close_log,int log_buf_size,int split_lines,int max_queue_size)
{
    ...
    if(index == log_full_name.size())
        log_full_name += std::to_string(my_tm.tm_year+1900)+"_"+std::to_string(my_tm.tm_mon+1)+"_"+std::to_string(my_tm.tm_mday);
    else
    {
        dir_name = file_name.substr(0,index+1);
        log_full_name += dir_name + std::to_string(my_tm.tm_year+1900)+"_"+std::to_string(my_tm.tm_mon+1)+"_"+std::to_string(my_tm.tm_mday) +"_"+ log_name;
    }
